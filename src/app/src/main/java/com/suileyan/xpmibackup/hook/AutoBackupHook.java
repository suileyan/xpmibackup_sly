...
public class AutoBackupHook {
     ...
     * 首次开启时包列表在后台收集（HIGH-16），避免 PackageManager 耗时阻塞主线程
     */
    private static void writeNasAutoBackupBasePreferences(Context context, boolean enabled, String deviceId, String deviceName) {
        ...
        if (prefs.getStringSet(PREF_KEY_NAS_PACKAGES, null) == null) {
            collectUserPackagesAsync(context);
        }
    }

    /**
     * 后台收集第三方应用包名并写入偏好；完成后重新刷新调度
     * 避免 getInstalledApplications 在主线程执行导致 ANR（HIGH-16），共享守护线程池（NEW-H-01）
     */
    private static void collectUserPackagesAsync(Context context) {
        var appContext = context != null ? context.getApplicationContext() : AutoBackupHook.appContext;
        if (appContext == null) {
            return;
        }
        com.suileyan.comm.Async.run("collect-packages", () -> {
            try {
                var packages = collectUserPackages(appContext);
                getAutoBackupPreferences(appContext).edit().putStringSet(PREF_KEY_NAS_PACKAGES, packages).apply();
                if (isNasAutoBackupEnabled(appContext)) {
                    refreshNasAutoBackupSchedule(appContext);
                }
            } catch (Throwable e) {
                logError("AutoBackupHook: collect user packages failed", e);
            }
        });
    }
    ...
    /** 主线程 Handler 复用，避免每次刷新调度都新建（MED-18） */
    ...
    private static void refreshNasAutoBackupScheduleDelayed() {
        ...
        MAIN_HANDLER.postDelayed(() -> {
            try {
                refreshNasAutoBackupSchedule(context);
                refreshNasAutoBackupPreferenceViews(context);
            } catch (Throwable e) {
    ...
    /**
     * 读取备份应用原生偏好文件
     * MODE_MULTI_PROCESS 在 Android N+ 已废弃且不可靠（MED-34），移除该 flag
     */
    private static android.content.SharedPreferences getAutoBackupPreferences(Context context) {
        return context.getSharedPreferences(context.getPackageName() + PREF_NAME_SUFFIX, Context.MODE_PRIVATE);
    }
     ...
     * onPreferenceClick 定义在 Preference 类上，必须在 preference 对象上调用（HIGH-15）
     */
    ...
    /**
     * 显示备份项目选择弹窗并保存包名集合
     * 包列表在后台收集（HIGH-16），收集完成后回到主线程构建对话框，避免 ANR
     */
    private static void showItemsDialog(Context context, Object preference) {
        var appContext = context != null ? context.getApplicationContext() : null;
        if (appContext == null) {
            return;
        }
        // 共享守护线程池收集包列表（NEW-H-01）
        com.suileyan.comm.Async.run("items-dialog", () -> {
            try {
                var appMap = collectUserPackageLabels(appContext);
                var labels = appMap.keySet().toArray(new String[0]);
                var packages = appMap.values().toArray(new String[0]);
                var selectedPackages = getNasBackupPackages(appContext);
                var checked = new boolean[packages.length];
                for (var i = 0; i < packages.length; i++) {
                    checked[i] = selectedPackages.contains(packages[i]);
                }
                var uiContext = context;
                var uiPreference = preference;
                // 复用静态主线程 Handler（NEW-L-19）
                MAIN_HANDLER.post(() -> {
                    try {
                        showItemsDialogOnUi(uiContext, uiPreference, labels, packages, checked);
                    } catch (Throwable e) {
                        logError("AutoBackupHook: show items dialog on UI failed", e);
                    }
                });
            } catch (Throwable e) {
                logError("AutoBackupHook: collect packages for dialog failed", e);
            }
        });
    }

    /** 在主线程构建项目选择对话框 */
    private static void showItemsDialogOnUi(Context context, Object preference, String[] labels, String[] packages, boolean[] checked) {
