...
1. 电脑双击 `dist\mibackpc.exe`，自动打开控制页 `http://127.0.0.1:8321/`
...
USB 通道（**优先于局域网**）：装有 adb 并连上手机后，后台协程在「设备在线但通道未建立」
时**自动执行 adb reverse**，无需手动操作；控制页「重连」按钮用于强制重建。
手机侧发现 USB 通道（`127.0.0.1:8321` 有应答）时，会把「电脑备份」方案的地址切到 USB
（仅当电脑名与已配对的一致），拔线后自动回落局域网，全程无需重新配对。
通道状态由后台协程按 15s → 60s 指数退避探测（`adb reverse --list` 探测 3s 硬超时、
执行 `adb reverse` 5s 硬超时），
控制页 1.2s 的轮询只读缓存、不 spawn 进程；失败原因（无 adb / 无设备 /
未授权 / 未建立）直接显示在卡片上。

命令行参数：`-port 8321` `-root D:\备份目录` `-no-browser`

> **配置与落地目录都相对 exe 所在目录，不是当前工作目录。**
> 配置固定读 exe 同级的 `mibackpc.json`（`configPath()` = `filepath.Dir(os.Executable())`），
> 备份目录首次默认为 exe 同级的 `mibackups/`；且**每次启动都会重写一次配置**
> （`-port` / `-root` 会被持久化进去）。所以换目录放 exe 等于换掉整套配置——
> 全仓库只保留 `pc/dist/` 一份，不要再在 `pc/` 根另放 exe，否则会出现
> 「配置改了没生效」或两个实例各自读一份配置的假象。
...
- **单实例约束**：发现端口是**独占绑定**，第二个实例会绑定失败并在日志里
  ...
  **不会**静默共享端口——同端口多 socket 会让发现通道静默失聪。
  同一台电脑只保留 `pc/dist/` 下的这一份 exe；更换 exe 时先结束旧进程。
...
- `/dav/*` 需要 Basic 认证（首次启动随机生成密码，保存在 exe 同级的 `mibackpc.json`）；
  密码仅在配对被电脑端明确同意后下发
...
产物固定输出到 `dist/`（`pc/dist/` 已在 `.gitignore` 中，**仓库不落二进制**）；
运行时生成的 `mibackpc.json` 与默认 `mibackups/` 也在 `dist/` 旁。
